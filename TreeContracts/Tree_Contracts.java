...
class AbsTree {
	...
	@Requires({"member(n)" , "!(get_count()==1 && left == null && right == null && n==value)"})		
...
class AbsTree_Iterator {
	...
	@Requires("root.ordered()")							//------input tree is ordered-------
	@Ensures("stack.peek() == root.min()")				//------stack-top has the minimum value in the tree-------
	...
	@Requires("hasNext()")										//------- there are more values in the tree/duptree--------
	@Ensures("stack.isEmpty() || result >= old(value)")			//-------  next value maintains the ascending order -------
	public int next() {
		if (count == 0) {
	...
	@Requires("true")									 
	@Ensures("node==null || node.min().value == stack.peek().value") 	//----- the next smallest value in the tree/duptree is at the top of the stack
